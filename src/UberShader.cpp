...
UberShader::UberShader()
  ...
  mShader.define("VECTOR", to_string(ETonemap::Vector));
  mShader.define("FLOW", to_string(ETonemap::Flow));
  mShader.define("FALSE_COLOR_PPG", to_string(ETonemap::FalseColorPPG));
    ...
    /// Shader version from original PPG code. For comparability of representation
      vec3 falseColorPPG(float v) {
        ...
        vec3 hsvToRgb(float h, float s, float v){
                    ...
                    float offset = v-c;
                    if(hp<1.0){
                        return vec3(c, x, 0)+offset;
                    }else if(hp<2.0){
                        return vec3(x,c,0)+offset;
                    }else if(hp<3.0){
                        return vec3(0,c,x)+offset;
                    }else if(hp<4.0){
                        return vec3(0,x,c)+offset;
                    }else if(hp<5.0){
                        return vec3(x,0,c)+offset;
                    }else if(hp<6.0){
                        return vec3(c,0,x)+offset;
                    }else{
                        return vec3(0,0,0)+offset;
                    }
                }
        ...
        vec3 applyTonemap(vec3 col) {
            switch (tonemap) {
                ...
                case VECTOR:    //col=vec3(col.x==-0?0:col.x, col.y==-0?0:col.y, col.z==-0?0:col.z);
                //return hsvToRgb(acos(col.z/length(col))*360.0/M_PI, col.x==0?(col.y>0?M_PI+M_PI/2.0:M_PI/2.0):(atan(col.y, col.x)+M_PI)*360.0/(2*M_PI), length(col));
                //col=col/(applyExposureAndOffset(2*vec3(1,1,1))-applyExposureAndOffset(2*vec3(-1,-1,-1)));
                col=applyInverseExposureAndOffset(col);
                if (col.x==0)
                    col.x=0;
                if (col.y==0)
                    col.y=0;
                ...
                if (length(col)==0)
                    return col;
                col = col/length(col);
                return applyExposureAndOffset(vec3(sRGB(0.5*(1.0+col.r)), sRGB(0.5*(1.0+col.g)), sRGB(0.5*(1.0+col.b))));
                case FLOW:col=applyInverseExposureAndOffset(col);
                if (col.x==0)
                    col.x=0;
                if (col.y==0)
                    col.y=0;
                ...
                if (length(col)==0)
                    return col;
                col = col/length(col);
                float cosTheta = min(max(-1.0, col.z), 1.0);
                float phi = col.x==0?(col.y>0?M_PI/2.0:-M_PI/2.0):atan(col.y, col.x);
                while(phi<0)
                    phi += M_PI*2.0;
                cosTheta = (cosTheta+1.0)*0.5;
                phi=phi/(M_PI*2.0);
                vec3 r= applyExposureAndOffset(hsl2rgb(vec3(phi, cosTheta,0.5)));
                return vec3(sRGB(r.x), sRGB(r.y), sRGB(r.z));
            case FALSE_COLOR_PPG:
            return vec3(falseColorPPG(pow(col.r, 1.0f/2.2f)));
            }
            return vec3(0.0);
        }
